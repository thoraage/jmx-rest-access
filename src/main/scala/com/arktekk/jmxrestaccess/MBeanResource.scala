...
object MBeanResource extends RestHelper {
  serve {
    case req@Req(MBeansUri(host, Nil), _, GetRequest) => contain {Some(getAll(req, host, null))}
    case req@Req(DomainMBeansUri(host, domainName), _, GetRequest) => contain {Some(getAll(req, host, domainName))}
    case req@Req(MBeanUri(host, domainAndKeys, Nil), _, GetRequest) => contain {Some(get(req, host, domainAndKeys))}
  }
